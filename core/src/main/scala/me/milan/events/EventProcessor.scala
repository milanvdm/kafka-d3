...
import me.milan.config.{KafkaConfig, AggregateStoreConfig}
...
object EventProcessor {
  ...
  def kafka[F[_]: Concurrent, A >: Null: AvroSerde, E >: Null: AvroSerde](
    ...
    SignallingRef[F, Boolean](false).map { haltSignal =>
      new KafkaEventProcessor[F, A, E](kafkaConfig, writeSideConfig, registry, aggregator, name, from, to, timeToLive, haltSignal)
    }
...
class KafkaEventProcessor[F[_]: Concurrent, A >: Null: AvroSerde, E >: Null: AvroSerde](
  ...
  override def clean: F[Unit] = Sync[F]
