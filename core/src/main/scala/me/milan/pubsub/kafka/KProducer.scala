...
class KProducer(config: KafkaConfig) {
  ...
  producerProps.put(ProducerConfig.BOOTSTRAP_SERVERS_CONFIG, config.bootstrapServers.show)
  producerProps.put(ProducerConfig.CLIENT_ID_CONFIG, Key.generate.value)
  producerProps.put(AbstractKafkaAvroSerDeConfig.SCHEMA_REGISTRY_URL_CONFIG, config.schemaRegistry.url.renderString)
